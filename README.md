...
- **Chained** — stacks MRs instead of fanning out: ticket N forks from the head of
  ...
  verdict, not a more optimistic one. For turning it on, see
...
any agent runs), and per chainable base the stack as it stands: `wouldForkFrom`,
the MR list, and any `rivals` (two MRs on the same base — the most recent won).
